...
function get_leader_config () {
  ...
  LEADER_CONFIG=`cat $ROOT_DIR/config-files/leader`
...
function get_proxy_config () {
  ...
  PROXY_CONFIG=`cat $ROOT_DIR/config-files/kube-proxy.yml`
...
function get_kube_config () {
  ...
  KUBE_CONFIG=`cat $ROOT_DIR/config-files/kubeconfig.yml`
...
function change_yml_file () {
  ...
  if [[ "$check_master" == "master" ]] ; then
    YML_FILE="$ROOT_DIR/config-files/tmp-master.yml"
  else
    YML_FILE="$ROOT_DIR/config-files/tmp-node.yml"
  ...
  USER_HOME=$(echo "$HOME")
  DOCKER_CFG=$(cat $USER_HOME/.docker/config.json | $ROOT_DIR/bin/trim.sh)
  DOCKER_CFG2=$(echo ${DOCKER_CFG} | sed "s/\:/\\\:/g" | sed 's.\/.\\/.g')
  DISCOVERY_URL=`cat $ROOT_DIR/config-files/DISCOVERY_URL`
  ...
  sed "s/DISCOVERY_URL/${DISCOVERY_URL3}/g" $YML_FILE
  sed "s/DOCKER_CONFIG_DATA/'${DOCKER_CFG2}/g" $YML_FILE

  # sed "s/KUBE_CONFIG_DATA/${KUBE_CONFIG}/g" $YML_FILE
  # sed "s/PROXY_CONFIG_DATA/${PROXY_CONFIG}/g" $YML_FILE
  # sed "s/LEADER_CONFIG_DATA/${LEADER_CONFIG}/g" $YML_FILE
  echo $YML_FILE
}
...
function work_on_droplet () {
  ...
  id=$(echo $txt | $ROOT_DIR/bin/JSON.sh -b | egrep '\["droplet","id"\]' | xargs echo | awk '{x=$2}END{print x}')
  ...
  while : ; do
    ...
    info=$(curl -X GET "https://api.digitalocean.com/v2/droplets/${id}" \
           -H 'Content-Type: application/json' \
           -H "Authorization: Bearer $DO_TOKEN")
    private_ip=$( echo $info | $ROOT_DIR/bin/JSON.sh -b | egrep '\["droplet","networks","v4",0,"ip_address"\]' | xargs echo | awk '{x=$2}END{print x}')
    public_ip=$(echo $info | $ROOT_DIR/bin/JSON.sh -b | egrep '\["droplet","networks","v4",1,"ip_address"\]' | xargs echo | awk '{x=$2}END{print x}')
    echo "~~~~~~~~~~~~~~~~~~~~~~~~"
